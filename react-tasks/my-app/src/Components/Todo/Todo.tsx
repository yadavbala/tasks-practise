...
import {ErrorContainer,InputContainer,ButtonContainer,MainContainer} from '../Login/Login'
  ...
  return(
      <MainContainer>
            ...
            <form onSubmit={handleSubmit}>
                <InputContainer type='text' value={todo} minLength={10} onChange={(event)=>handleTodoChange(event)}/>
                <Spacing><ButtonContainer type='submit'>submit</ButtonContainer></Spacing>
            ...
            <List>
            {
                ...
                return (
                    <div key={i}>
                      <ListItems onDoubleClick={()=>removeList(ele)}>{`${++i} ${ele}\u00A0\u00A0\u00A0\u00A0${new Date().getDate()} ${months[new Date().getMonth()]}`}</ListItems>
                    </div>
                 )
                })
